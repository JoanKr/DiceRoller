...
// to this Activity
        ...
        val isHoldEnabled = intent.getBooleanExtra(getString(R.string.hold_enable_key), true) // Apply the current isHoldEnabled setting
        binding.enableHoldSwitch.isChecked = isHoldEnabled
// Select the current num dices value from the spinner. // We assume there are 5 dices max. and 2 dices min.
        if (numDice in 2..5)
...
// that will indicate the number of dices
            val spinnerSelection = binding.numDiceSpinner.selectedItem.toString().toInt() // get the current state of the hold enable switch
            val holdEnable = binding.enableHoldSwitch.isChecked
// Create an empty intent that will be used to pass data to the calling Activity val result = Intent().apply {
